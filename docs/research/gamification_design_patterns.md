...
* **Case Studies**:
  * **LinkedIn (2024)**: Quietly retired its *Community Top Voice* gold badges. Internal data showed that badge-motivated users produced quantity over quality, spamming posts to chase the badge rather than sharing genuine expertise.
  * **Foursquare (2014)**: Scrapped its famous mayorships and badge system. The gamification successfully drove check-ins, but failed to drive the discovery and search behaviors that the core business model actually needed.
  * **Google News**: Defunct badge system killed for the exact same reason—driving clickbait interactions rather than authentic news consumption.

### 2. Winnable Hyper-Local Competitions
...
* **Case Studies**:
  ...
  * **Duolingo (Good/Mitigated)**: Duolingo runs on streaks but wraps them in **agency**. Users choose their daily goal level, can buy or earn "streak freezes" to pause the pressure, and have control over the mechanic.

### 5. Variable Reward Magnitude (Anticipation vs. Loss Aversion)
* **The Concept**: Designing reward cycles around anticipation (what is coming next) rather than loss aversion (fear of losing what you built).
* **Why it Works**: The gap between knowing a reward is coming and not knowing how large it will be is one of the strongest engagement signals in design. Dopamine is released during the *anticipation* phase.
* **Case Study (Good)**:
...
* **The Concept**: Leveraging the Gestalt principle of closure—the brain's hardwired tendency to perceive incomplete visual patterns as loops that must be closed.
* **Why it Works**: Visualizing progress as a near-complete shape (e.g., a 90% closed circle) creates an open loop in the user's mind, driving them to finish the task.
* **Case Study (Good)**:
...
| **Duolingo**    | Flexibly Managed Streaks                  | 🟢 Good (User Agency)       | Empowers users with choice (goal adjustments, streak freezes) to prevent loss-aversion burnout.                            |
...
### 4. Gestalt Closure (For Progress & Onboarding)
